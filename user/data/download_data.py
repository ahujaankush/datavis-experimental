    ...
    for unit in units:
        for waste_operation in waste_operations:
            ...
            for index, row in filtered_data.iterrows():
                store = {}
                store['year'] = row['TIME_PERIOD']
                store['value']= row['OBS_VALUE']
                datapoints.append(store)

            if (len(datapoints) == 0): # Filter out tables with no data
                continue

            # Export datapoints to JSON file
            filename = f"{current_directory}/eu/{country}_{waste_operation}_{unit}.json"  # Example file name: "US_kg_COL.json"
